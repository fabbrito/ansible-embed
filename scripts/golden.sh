...
#
# What this buys that `make check` cannot: ansible-lint and the tool's own
# validator both accept configs that are wrong in ways only the BYTES show — a
# trust block on a host nothing fronts, a body cap silently clamped by a
# matcher-less default, a redaction filter naming a prefix that never occurs.
