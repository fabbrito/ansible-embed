...
- rclone's `install.sh` overwrites `/usr/bin/rclone` behind dpkg, unpinned and unverified; any apt touch of `rclone`
  reverts it.
- A `.deb` through dpkg records the real version, and the committed sha256 is checked on every converge, so the board
  needs no gpg key.

## Bump

On the controller, never on a board. rclone's release signing key fingerprint is
...
gpg --decrypt SHA256SUMS 2>/dev/null | grep -E "linux-(arm-v7|arm64)\.deb"
```
