...
This repo converges nothing. It ships what SOMEBODY ELSE'S converge will do, to
boards you will never see — so say what moves on their boards once they adopt the
tag: what a re-run does, what restarts or is recreated, and whether an
already-converged board changes. Name anything left as an accepted risk.
...
- [ ] No secrets, credentials, hostnames or addresses — and nothing naming a consumer, a client, or their vendor
...
`--check --diff` against a real board with the diff read, and a second converge
