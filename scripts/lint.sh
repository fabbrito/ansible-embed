...
#
# This repo is a collection, not a control node: no inventory, no board. The
# dry-run leg the consuming repo runs (TEST_HOST=<board>, --check --diff) has no
...
# resolves fabbrito.embed.* against the working tree, uncommitted edits
...
ln -sfn "$PWD" "$staged/embed" || exit 1
